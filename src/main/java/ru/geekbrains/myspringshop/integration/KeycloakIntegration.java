package ru.geekbrains.myspringshop.integration;
...
import ru.geekbrains.myspringshop.integration.dto.CreateUserRequest;
import ru.geekbrains.myspringshop.integration.dto.JwtToken;
...
public class KeycloakIntegration {
    ...
    public void createUser(Person person) {
        ...
        var response = restTemplate.exchange(
                KEYCLOAK_ADDRESS + "/auth/admin/realms/master/users",
                HttpMethod.POST,
                entity,
                Void.class
        );
    ...
    public JwtToken authorize(String username, String password) {
        ...
        var response = restTemplate.exchange(KEYCLOAK_ADDRESS + "/auth/realms/master/protocol/openid-connect/token",
                HttpMethod.POST, entity, JwtToken.class);

        if(response.getStatusCode() == HttpStatus.UNAUTHORIZED) {
            throw new RuntimeException("Пришла 401 ошибка, неправильные логин и пароль");
        } else {
